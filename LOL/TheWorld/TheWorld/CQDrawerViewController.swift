...
class CQDrawerViewController: UIViewController {
    ...
    // 滑动宽度
    var leftWidth:CGFloat = UIScreen.mainScreen().bounds.size.width * 0.85
    var rightWidth:CGFloat = 200
    // 缩放大小
    var leftScaleHeigth:CGFloat = 0
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        // 当前控制器是父类
        leftController?.view.frame = CGRectMake( -UIScreen.mainScreen().bounds.size.width * 0.85, UIScreen.mainScreen().bounds.height, UIScreen.mainScreen().bounds.size.width * 0.85, UIScreen.mainScreen().bounds.size.height);
        ...
        self.mainController?.view.addObserver(self, forKeyPath: "frame", options: NSKeyValueObservingOptions.New, context: nil);
        // 绑定手势
        let recoginizer = UIPanGestureRecognizer.init(target: self, action: #selector(CQDrawerViewController.handlePan(_:)))
        self.view.addGestureRecognizer(recoginizer)
        let tap1 = UITapGestureRecognizer.init(target: self, action: #selector(CQDrawerViewController.handleTap(_:)))
        let tap2 = UITapGestureRecognizer.init(target: self, action: #selector(CQDrawerViewController.handleTap(_:)))
        self.leftController?.view.addGestureRecognizer(tap1)
        self.rightController?.view.addGestureRecognizer(tap2)
        // 通知
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(CQDrawerViewController.showLeftSide), name: "showleft", object: nil)
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(CQDrawerViewController.showRightSide), name: "showright", object: nil)
    ...
    func showLeftSide() {
        ...
        UIView.animateWithDuration(0.2, animations: { () -> Void in
            self.mainController?.view.frame = CGRectMake(self.leftWidth, self.leftScaleHeigth, (self.view.frame.width), (self.view.frame.height)-2*self.leftScaleHeigth)
        })
    }
    func showRightSide() {
        ...
        UIView.animateWithDuration(0.2, animations: { () -> Void in
            self.mainController?.view.frame = CGRectMake(-self.rightWidth, self.rightScaleHeigth, (self.view.frame.width), (self.view.frame.height)-2*self.rightScaleHeigth)
        })
    ...
    func handlePan(recognizer:UIPanGestureRecognizer) {
        ...
        if recognizer.state == UIGestureRecognizerState.Began {
            ...
            height -= 2*y
            UIView.animateWithDuration(0.2, animations: { () -> Void in
                self.mainController?.view.frame = CGRectMake(flag*(self.originx + x), y, (self.mainController?.view.frame.width)!, height)
                },completion: { (bool:Bool) -> Void in
                    if (self.originx + x == 0) {
                        self.leftController?.view.removeFromSuperview()
                        self.rightController?.view.removeFromSuperview()
    ...
    override func observeValueForKeyPath(keyPath: String?, ofObject object: AnyObject?, change: [String : AnyObject]?, context: UnsafeMutablePointer<Void>) {
        if keyPath == "frame" {
            self.leftController?.view.frame = CGRectMake(-UIScreen.mainScreen().bounds.width * 0.85 + (self.mainController?.view.frame.origin.x)!,0, UIScreen.mainScreen().bounds.size.width * 0.85, UIScreen.mainScreen().bounds.size.height);
